...
NGame = {
	...
	FUEL_RESOURCE = "rubber",							-- resource that will give country fuel
	...
	HANDS_OFF_START_TAG = "SNW",					-- tag for player country for -hands_off runs. use an existing tag that is less likely to affect the game
...
NDiplomacy = {
	...
	TENSION_FORCE_GOVERNMENT = 0,                -- Amount of tension generated by forcing government (per state)
	TENSION_VOLUNTEER_FORCE_DIVISION = 0,			-- Amount of tension generated for each sent division
	TENSION_DECAY_DAILY = 0,								-- Each months tension decays this much
	...
	TENSION_TIME_SCALE_MIN = 0,						-- Timed tension scale won't decrease under this value
	TENSION_GUARANTEE = 0,
...
NCountry = {
	...
	POPULATION_YEARLY_GROWTH_BASE = 0.0,			-- basic population growth per year, used for monthly manpower gain
	...
	BASE_STABILITY_WAR_FACTOR = -0.25,				-- Default stability war factor
	...
	BOMBING_WAR_SUPPORT_PENALTY_SCALE = 0.0, 			-- Scaling of bomber damage to war support impact, will be added weekly as a war support penalty
	...
	HEROES_BEING_KILLED_WAR_SUPPORT_PENALTY_SCALE = 0.0,			-- Scaling of war heroes manpower lost to war support impact, will be added weekly as a war support penalty
	...
	HEROES_BEING_KILLED_WEEKLY_WAR_SUPPORT_PENALTY_DECAY = 0.1,	-- Weekly decay of war heroes manpower lost war support penalty
	...
	CONVOYS_BEING_RAIDED_WAR_SUPPORT_PENALTY_SCALE = 0.0,			-- Scaling of trade convoy raided to war support impact, will be added weekly as a war support penalty
	...
	BASE_FUEL_GAIN_PER_OIL = 10,						-- base amount of fuel gained hourly per excess oil
...
NResistance = {
	INITIAL_STATE_RESISTANCE = 20.0,						-- initial resistance percentage of a state once it is captured
...
NProduction = {
	...
	MAX_CIV_FACTORIES_PER_LINE = 25,	-- 15 #Max number of factories that can be assigned a single production line.
	...
	BASE_FACTORY_MAX_EFFICIENCY_FACTOR = 10,	-- Base max efficiency for factories expressed in %.
	...
	MIN_FIELD_TO_TRAINING_MANPOWER_RATIO = 0.5,	-- Ratio which % of army in field can be trained
...
NTechnology = {
	...
	BASE_TECH_COST = 200,					-- Base cost for a tech. multiplied with tech cost and ahead of time penalties
...
NPolitics = {
	...
	BASE_POLITICAL_POWER_INCREASE = 1,	-- Weekly increase of PP.
...
NBuildings = {
	...
	RADAR_RANGE_MIN = 40,				-- Radar range (from state center to province center) in measure of map pixels. Exluding techs.
	RADAR_RANGE_MAX = 100,				-- Range is interpolated between building levels 1-15.
	RADAR_INTEL_EFFECT = 120,			-- Province covered by radar increases intel by 10 (where 255 is max). Province may be covered by multiple radars, then the value sums up.
	...
	BASE_FACTORY_REPAIR = 1.0,			-- 0.3 Default repair rate before factories are taken into account
	BASE_FACTORY_REPAIR_FACTOR = 2.0,	-- 2.0 Factory speed modifier when repairing.
	SUPPLY_PORT_LEVEL_THROUGHPUT = 200,--20,   -- supply throughput per level of naval base
	...
	INFRASTRUCTURE_RESOURCE_BONUS = 0.05, -- multiplactive resource bonus for each level of (non damaged) infrastructure
...
NMilitary = {
	...
	INFRASTRUCTURE_MOVEMENT_SPEED_IMPACT = 0.0,	-- speed penalty per infrastucture below maximum.
	...
	FIELD_EXPERIENCE_ON_DIVISION_MULT = 1.03,	-- Multiply field experience gained by this, when applying to divisional commander
	...
	CASUALTY_COUNT_FOR_HISTORY_ENTRY = 1000,	-- number of received casualties to receive a history entry (one only)
	...
	WAR_SCORE_LOSSES_RATIO = 5,									-- war score gained for every 1000 casualties
	...
	LAND_COMBAT_STR_ARMOR_ON_SOFT_DICE_SIZE = 12,   -- extra damage dice if our armor outclasses enemy
	LAND_COMBAT_ORG_ARMOR_ON_SOFT_DICE_SIZE = 6,    -- extra damage dice if our armor outclasses enemy
	LAND_COMBAT_STR_ARMOR_DEFLECTION_FACTOR = 0.5, -- % damage taken if armor outclassing enemy
	LAND_COMBAT_ORG_ARMOR_DEFLECTION_FACTOR = 0.75, -- % damage taken if armor outclassing enemy
	...
	LAND_COMBAT_FORT_DAMAGE_CHANCE = 5,		-- (was 5) chance to get a hit to damage on forts. (out of 100)
	...
	ATTRITION_EQUIPMENT_PER_TYPE_LOSS_CHANCE = 0.01, -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	BASE_CHANCE_TO_AVOID_HIT = 75,                 -- Base chance to avoid hit if defences left.
	CHANCE_TO_AVOID_HIT_AT_NO_DEF = 50,	           -- chance to avoid hit if no defences left.
	COMBAT_MOVEMENT_SPEED = 0.5,	               -- speed reduction base modifier in combat
	...
	INITIATIVE_PICK_COUNTER_ADVANTAGE_FACTOR  = 1, --0.35, -- advantage per leader level for picking a counter
	AMPHIBIOUS_INVADE_MOVEMENT_COST = 24.0,        -- total progress cost of movement while amphibious invading
	LAND_SPEED_MODIFIER = 0.1,					   -- basic speed control
	...
	BASE_FORT_PENALTY = -0.025, 					   -- fort penalty -100% max
	...
	SUPPLY_GRACE = 24 * 7,							   -- troops always carry 1 week of food and supply
	...
	SUPPLY_ORG_MAX_CAP = 0.50,                     -- Max organization is factored by this if completely out of supply
	...
	OUT_OF_SUPPLY_MORALE = -0.1,                   -- max org regain reduction from supply
	...
	PLANNING_DECAY = 0.025,
	PLAYER_ORDER_PLANNING_DECAY = 0.001,				-- Amount of planning lost due to player manual order
	PLANNING_GAIN = 0.02,
	...
	COMBAT_OVER_WIDTH_PENALTY = -0.7,					-- over combat width penalty per %.
	...
	ARMOR_VS_AVERAGE = 0.2,			            -- how to weight in highest armor & pen vs the division average
	PEN_VS_AVERAGE = 0.5,
	...
	NEW_COMMANDER_RANDOM_PERSONALITY_TRAIT_CHANCES = {  -- chances to gain a personality trait for new generals
		1, -- 100% will be generic in on_action
	...
	UNIT_LEADER_MODIFIER_COOLDOWN_ON_GROUP_CHANGE = 30,		-- time in days for a unit leader to regain its modifiers
...
NAir = {
	...
	TOP_SPEED_DAMAGE_BONUS_FACTOR = 0.01,				-- A factor for scaling the top speed of a plane into damage buff
	...
	COMBAT_BETTER_SPEED_DAMAGE_INCREASE = 0.65, 		-- How much the better Speed (than opponent's) can increase our damage to them.
	...
	AIR_REGION_SUPERIORITY_PIXEL_SCALE = 0.02,           -- air superiority scale = superiority/(pixels*this)
	...
	DETECT_CHANCE_FROM_AIRCRAFTS_EFFECTIVE_COUNT = 100, -- Max amount of aircrafts in region to give full detection bonus.
	...
	AIR_WING_XP_LEVELS = { 100, 200, 300, 400, 500, 600, 700, 800 }, 						--Experience needed to progress to the next level
...
NNavy = {
	...
	COMBAT_EVASION_TO_HIT_CHANCE_TORPEDO_MULT = 5.0,				-- the above evasion hit chance is multiplied by 400% if shooting with torpedos. Torpedoes are slow, so evasion matters more.
	...
	COMBAT_ARMOR_PIERCING_CRITICAL_BONUS = 0.5,						-- Bonus to critical chance when shooter armor piercing is higher then target armor.
	...
	NAVAL_RANGE_TO_INGAME_DISTANCE = 2.0,							-- Scale the ship stats "naval_range" to the ingame distance
	...
	AGGRESION_MULTIPLIER_FOR_COMBAT = 0.8,				-- ships are more aggresive in combat
	...
	AGGRESSION_TORPEDO_EFFICIENCY_ON_LIGHT_SHIPS = 0.4,   -- ratio for scoring for different gun types against light ships
	...
	MIN_ORG_ON_MANUAL_MOVE = 1.0,	-- org will clamped to this ratio on manual move
	...
	SUPREMACY_PER_SHIP_PER_IC = 0.0005,
	...
	CONVOY_DETECTION_CHANCE_BASE = 2.0,								-- regular convoy base chance detection percentage (if this fails, no detection is done on that tick)
	...
	UNIT_TRANSFER_DETECTION_CHANCE_BASE = 4.0,							-- unit transfer and naval invasion base chance detection percentage (if this fails, no detection is done on that tick)
	...
	MIN_GUN_COOLDOWN = 0.5,											-- minimum cooldown for a gun	
	...
	SCREEN_RATIO_FOR_FULL_SCREENING_FOR_CONVOYS 					= 1.0,	-- this screen ratio to num convoys is needed for full screening beyond screen line
...
NAI = {
	...
	RESEARCH_AHEAD_OF_TIME_FACTOR = 0.0, 		-- To which extent AI should care about ahead of time penalties to research
	...
	MIN_AI_SCORE_TO_MOBILIZATION_LAW_OVERRIDE_HARD_CODED_SCORE = 1.0,
	MIN_AI_SCORE_TO_ECONOMY_LAW_OVERRIDE_HARD_CODED_SCORE = 1.0,
	...
	BASE_DISTANCE_TO_CARE = 100.0,				-- Countries that are too far away are less interesting in diplomacy
	...
	INVASION_COASTAL_PROVS_PER_ORDER = 32,				-- AI will consider one extra invasion per number of provinces stated here (num orders = total coast / this). The higher the less calculation. WARNING GAME FREEZES IF SET TO 1 !
	...
	HOUR_BAD_COMBAT_REEVALUATE = 96,                   -- if we are in combat for this amount and it goes shitty then try skipping it
	...
	LAND_DEFENSE_SUPPLY_HUB_IMPORTANCE = 100,             -- Strategic importance of supply hubs
	...
	CRITICAL_DECISION_PRIORITY = 500,					-- critical ai score for decisions, ai will be able to pick decisions if it has higher prio even if it is not time to pick them (0 to disable)
	...
	CONSTRUCTION_PRIO_CIV_FACTORY = 0.0,                                       -- base prio for civilian factories in the construction queue
	CONSTRUCTION_PRIO_MIL_FACTORY = 0.50,                                       -- base prio for military factories in the construction queue
	...
	CONSTRUCTION_PRIO_UNSPECIFIED = 2.00,                                       -- base prio for unspecified buildings (none of the categories above) in the construction queue
	...
	BUILDING_TARGETS_BUILDING_PRIORITIES = {				-- buildings in order of pirority when considering building targets strategies. First has the greatest priority, omitted has the lowest. NOTE: not all buildings are supported by building targets strategies.
		'farm', 'lumber_mill', 'carpentry', 'mine', 'quarry', 'arms_factory', 'ranch',
	},
	...
	ARMY_LEADER_ASSIGN_DEFENSE_MAX_DIG_IN_FACTOR = 2.0,         -- If defensive army, importance of general's MAX_DIG_IN_FACTOR modifier
	...
	ARMY_LEADER_ASSIGN_PLANNING_SPEED = 1.0,                    -- Importance of general's PLANNING_SPEED modifier
	ARMY_LEADER_ASSIGN_MAX_PLANNING = 1.0,                      -- Importance of general's MAX_PLANNING modifier
	ARMY_LEADER_ASSIGN_RECON_FACTOR = 0.0,                      -- Importance of general's RECON_FACTOR modifier
	...
	ARMY_LEADER_ASSIGN_SUPPLY_CONSUMPTION_FACTOR = 40.0,         -- Importance of general's SUPPLY_CONSUMPTION_FACTOR modifier
	ARMY_LEADER_ASSIGN_LAND_REINFORCE_RATE = 5.0,               -- Importance of general's LAND_REINFORCE_RATE modifier
...
NOperatives = {
	AGENCY_CREATION_DAYS = 180,						-- Number of days needed to create an intelligence agency
...
NSupply = {
	...
	CAPITAL_SUPPLY_BASE = 1.0, -- base supply for capital
	CAPITAL_SUPPLY_CIVILIAN_FACTORIES = 0.04, -- supply from one civilian factory
	CAPITAL_SUPPLY_MILITARY_FACTORIES = 0.05, -- supply from one military factory
	CAPITAL_SUPPLY_DOCKYARDS = 0.04, --supply from one naval factory
